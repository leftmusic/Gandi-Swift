...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    func applicationDidFinishLaunching(_ aNotification: Notification) {
        // 应用程序启动日志
        log("Gandi-Swift 应用程序启动")
        log("生产环境: \(isProduction ? "是" : "否")")
        ...
        if let resourcePath = bundle.path(forResource: "index", ofType: "html", inDirectory: "Pages") {
            ...
            log("从bundle资源中找到HTML文件: \(htmlFileURL?.path ?? "未知")")
        }
        ...
        if htmlFileURL == nil {
            ...
            if FileManager.default.fileExists(atPath: indexPath.path) {
                ...
                log("从bundle目录中找到HTML文件: \(indexPath.path)")
            }
        }
        ...
        if htmlFileURL == nil {
            ...
            if FileManager.default.fileExists(atPath: indexPath.path) {
                htmlFileURL = indexPath
                log("从可执行文件目录中找到HTML文件: \(indexPath.path)")
            }
        }
        ...
        if htmlFileURL == nil {
            ...
            if FileManager.default.fileExists(atPath: indexPath.path) {
                htmlFileURL = indexPath
                log("从构建目录中找到HTML文件: \(indexPath.path)")
            }
        }
        
        // 5. 尝试从Xcode构建目录查找（Gandi-Swift_Gandi-Swift.bundle/Contents/Resources/Pages/index.html）
        if htmlFileURL == nil {
            let bundleDir = executableDir.appendingPathComponent("Gandi-Swift_Gandi-Swift.bundle")
            let resourcesDir = bundleDir.appendingPathComponent("Contents").appendingPathComponent("Resources")
            let pagesDir = resourcesDir.appendingPathComponent("Pages")
            let indexPath = pagesDir.appendingPathComponent("index.html")
            
            if FileManager.default.fileExists(atPath: indexPath.path) {
                htmlFileURL = indexPath
                log("从Xcode构建目录中找到HTML文件: \(indexPath.path)")
            }
        }
        ...
        if htmlFileURL == nil {
            ...
            if FileManager.default.fileExists(atPath: indexPath.path) {
                htmlFileURL = indexPath
                log("从开发目录中找到HTML文件: \(indexPath.path)")
            }
        }
        ...
        if let htmlFileURL = htmlFileURL, FileManager.default.fileExists(atPath: htmlFileURL.path) {
            ...
            log("成功加载HTML文件: \(htmlFileURL.path)")
        } else {
            // 文件不存在，显示错误信息
            let errorHTML = """
            <!DOCTYPE html>
            ...
            log("错误: 无法找到HTML文件", level: "ERROR")
            log("可执行文件路径: \(executablePath)", level: "ERROR")
            log("可执行文件目录: \(executableDir.path)", level: "ERROR")
        }
